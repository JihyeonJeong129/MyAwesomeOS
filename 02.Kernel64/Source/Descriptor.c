...
void kInitializeGDTTablesAndTSS(void) {
    GDTR* pstGDTR;
    GDTENTRY8* pstEntry;
    TSSSEGMENT* pstTSS;
    int i;
    ...
    // Generate 4-Segment for NULL, 64-bit CODE/DATA and TSS
...
// Reset TSS Segment info
void kInitializedTSSSegment(TSSSEGMENT* pstTSS){
    kMemSet(pstTSS, 0, sizeof(TSSSEGMENT));
    pstTSS->qwIST[0] = IST_STARTADDRESS + IST_SIZE;
    pstTSS->wIOMapBaseAddress = 0xFFFF;
}

// Initialized IDT Table
void kInitializeIDTTables(void){
    ...
    for(i=0; i<IDT_MAXENTRYCOUNT; i++){
        kSetIDTEntry(&(pstEntry[i]), kDummyHandler, GDT_KERNELCODESEGMENT,
